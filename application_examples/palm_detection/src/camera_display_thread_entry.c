...
void camera_display_thread_entry(void *pvParameters)
{
    ...
    while (true)
    {
         ...
         */
        {
            ...
            /* Compute letterbox parameters with integer math */
            /* scale = min(dst_w/src_w, dst_h/src_h) in fixed-point 16.16 */
            const int32_t scale_w = (dst_w << 16) / src_w;  /* 192/640 << 16 */
            const int32_t scale_h = (dst_h << 16) / src_h;  /* 192/480 << 16 */
            ...
            /* Fill entire buffer with zero-point value (-128 = 0.0 in float) */
            memset(dst, (uint8_t)(-128), (size_t)(dst_w * dst_h * 3));

            /* Inverse scale: src_coord = dst_coord * inv_scale >> 16 */
            ...
            for (int dy = 0; dy < resized_h; dy++)
            {
                ...
                for (int dx = 0; dx < resized_w; dx++)
                {
                    ...
                    /* Quantize: q = round(val/255 * 256) - 128 = val - 128 */
            ...
            /* Store preprocess meta for postprocess coordinate remapping */
