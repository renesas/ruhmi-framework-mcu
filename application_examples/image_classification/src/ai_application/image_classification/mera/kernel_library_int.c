...
inline void AffineQuantizeFloatToInt8(const float* input, int8_t* output, uint32_t size,
...
inline void AffineDequantizeUInt8ToFloat(const uint8_t* input, float* output, uint32_t size,
...
inline void AffineDequantizeInt8ToFloat(const int8_t* input, float* output, uint32_t size,
  int8_t input_zp, float input_scale) {
  for (uint32_t i = 0; i < size; i++) {
    const int32_t val = input[i];
    output[i] = input_scale * (val - input_zp);
...
// Supporting function for Add op
inline void BroadcastAddRecursiveDimensions(const int8_t* input1, size_t* input1_offset_p,
...
// Supporting function for Sub op
inline void SubBroadcast(const int8_t* input_data, const int8_t* broadcast_data,
