...
/* Persistent gesture state per detection slot (written by draw_landmark_points, read by print_gesture_info) */
...
static void draw_landmark_points(uint8_t det_idx)
{
    ...
    /* Classify gesture (result displayed in sidebar by print_gesture_info) */
    {
...
static void print_gesture_info(void)
{
    ...
    /* Show gesture for first active detection slot */
    for (uint8_t i = 0; i < AI_MAX_DETECTION_NUM; i++)
    {
        if (g_last_gesture[i] != GESTURE_UNKNOWN && g_last_gesture[i] < GESTURE_COUNT)
        {
            print_bg_font_18(d2_handle, 820, 520, DISPLAY_FONT_SCALING,
