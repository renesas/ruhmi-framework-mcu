...
bool main_loop_palm_detection()
{
    ...
    /* -----------------------------------------------------------------
     * Simple nearest-neighbor palm tracking
     * Assigns each new detection to the closest previous slot so that
     * slot indices stay consistent across frames (prevents gesture/
     * handedness state from jumping between hands).
     * ----------------------------------------------------------------- */
    {
        /* Previous frame palm centres per slot (camera pixel coords) */
        static float prev_cx[MAX_LANDMARK_HANDS] = {0};
        static float prev_cy[MAX_LANDMARK_HANDS] = {0};
        static int   prev_valid[MAX_LANDMARK_HANDS] = {0};

        if (num_dets > 0 && prev_valid[0])
        {
            /* Compute new detection centres in camera coords for matching */
            float new_cx[MAX_LANDMARK_HANDS];
            float new_cy[MAX_LANDMARK_HANDS];
            for (int i = 0; i < num_dets; i++)
            {
                new_cx[i] = palm_results[i].box_x * (float)CAM_VGA_WIDTH;
                new_cy[i] = palm_results[i].box_y * (float)CAM_VGA_HEIGHT;
            }

            /* For 2 detections: check if swapping gives smaller total distance */
            if (num_dets == 2 && prev_valid[1])
            {
                float dx0 = new_cx[0] - prev_cx[0];
                float dy0 = new_cy[0] - prev_cy[0];
                float dx1 = new_cx[1] - prev_cx[1];
                float dy1 = new_cy[1] - prev_cy[1];
                float dist_same = (dx0*dx0 + dy0*dy0) + (dx1*dx1 + dy1*dy1);

                float sx0 = new_cx[0] - prev_cx[1];
                float sy0 = new_cy[0] - prev_cy[1];
                float sx1 = new_cx[1] - prev_cx[0];
                float sy1 = new_cy[1] - prev_cy[0];
                float dist_swap = (sx0*sx0 + sy0*sy0) + (sx1*sx1 + sy1*sy1);

                if (dist_swap < dist_same)
                {
                    /* Swap palm_results[0] and [1] */
                    palm_box_t tmp = palm_results[0];
                    palm_results[0] = palm_results[1];
                    palm_results[1] = tmp;
                    float tc = new_cx[0]; new_cx[0] = new_cx[1]; new_cx[1] = tc;
                    tc = new_cy[0]; new_cy[0] = new_cy[1]; new_cy[1] = tc;
                }
            }
            else if (num_dets == 1 && prev_valid[1])
            {
                /* Only 1 detection but had 2 last frame — assign to nearest slot */
                float d0 = (new_cx[0]-prev_cx[0])*(new_cx[0]-prev_cx[0])
                         + (new_cy[0]-prev_cy[0])*(new_cy[0]-prev_cy[0]);
                float d1 = (new_cx[0]-prev_cx[1])*(new_cx[0]-prev_cx[1])
                         + (new_cy[0]-prev_cy[1])*(new_cy[0]-prev_cy[1]);
                if (d1 < d0)
                {
                    /* Move det 0 → slot 1 (leave slot 0 empty) */
                    palm_results[1] = palm_results[0];
                    memset(&palm_results[0], 0, sizeof(palm_box_t));
                    /* Adjust: slot 0 is empty, slot 1 has the detection */
                    num_dets = 2; /* tell downstream there are 2 slots (slot 0 empty) */
                }
            }

            /* Update prev centres */
            for (int i = 0; i < MAX_LANDMARK_HANDS; i++) prev_valid[i] = 0;
            for (int i = 0; i < num_dets; i++)
            {
                if (palm_results[i].box_size > 0.0f)
                {
                    prev_cx[i] = palm_results[i].box_x * (float)CAM_VGA_WIDTH;
                    prev_cy[i] = palm_results[i].box_y * (float)CAM_VGA_HEIGHT;
                    prev_valid[i] = 1;
                }
            }
        }
        ...
        {
            /* First frame or no detections — just store centres */
            for (int i = 0; i < MAX_LANDMARK_HANDS; i++) prev_valid[i] = 0;
            for (int i = 0; i < num_dets; i++)
            {
                prev_cx[i] = palm_results[i].box_x * (float)CAM_VGA_WIDTH;
    ...
    if (num_dets > 0)
    {
        ...
        for (int i = 0; i < num_dets; i++)
        {
            /* Skip empty slots (from tracking reorder) */
            if (palm_results[i].box_size <= 0.0f) continue;
        ...
        for (int i = 0; i < num_dets && num_hands < MAX_LANDMARK_HANDS; i++)
        {
            /* Skip empty slots (from tracking reorder) */
            if (palm_results[i].box_size <= 0.0f) {
                num_hands++;  /* keep slot index in sync */
                continue;
            }
